...
refXList0 = X(F<=floor(frameNUM/10));  % new
refYList0 = Y(F<=floor(frameNUM/10));  % new
% the 1st round of AIM, using the 1st subset (X,Y) as reference
[Xpdc, Ypdc, driftX1, driftY1] = IntersectionMax(X,Y,refXList0,refYList0,F,10,floor(frameNUM/10),imageWidth,IntersectD);  % new

refXList = Xpdc(F<=trackInterval);
refYList = Ypdc(F<=trackInterval);
% the 2nd round of AIM, using the 1st subset (X,Y) as reference
[Xpdc, Ypdc, driftX2, driftY2] = IntersectionMax(Xpdc,Ypdc,refXList,refYList,F,trackNUM,trackInterval,imageWidth,IntersectD);
% the 3rd round of AIM, using the entire subset (Xpdc,Ypdc) as reference
[Xpdc, Ypdc, driftX3, driftY3] = IntersectionMax(Xpdc,Ypdc,Xpdc,Ypdc,F,trackNUM,trackInterval,imageWidth,IntersectD);

LocAIM(:,1) = F;
LocAIM(:,2) = Xpdc;
LocAIM(:,3) = Ypdc;

Drift_x = driftX1 + driftX2 + driftX3; % new
Drift_y = driftY1 + driftY2 + driftY3; % new
